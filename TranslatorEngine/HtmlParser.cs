...
namespace TranslatorEngine
{
	public class HtmlParser
	{
		...
		public static string GetChineseContent(string htmlContent, bool needMarkChapterHeaders)
		{
			...
			{
				string text = array[i];
				if (!string.IsNullOrEmpty(text) && !text.StartsWith("#") && htmlContent.Contains(text))
				{
					string text2 = htmlContent.Substring(htmlContent.IndexOf(text) + text.Length);
					...
					{
						stringBuilder.AppendLine((needMarkChapterHeaders ? "$CHAPTER_HEADER$. " : "") + text2.Substring(0, text2.IndexOf("</" + text3 + ">")).TrimStart(" \u3000\t".ToCharArray()));
						break;
					}
				}
			}
			...
			for (int j = 0; j < array2.Length; j++)
			{
				...
				if (!string.IsNullOrEmpty(text4) && !text4.StartsWith("#") && htmlContent.Contains(text4))
				{
					string text5 = htmlContent.Substring(htmlContent.IndexOf(text4) + text4.Length);
					if (text4 != "<!--bodybegin-->")
					{
						...
						if (text5.Contains("</" + text6.TrimStart(new char[]
						{
							'/'
						}) + ">"))
						{
							stringBuilder.AppendLine(text5.Substring(0, text5.IndexOf("</" + text6.TrimStart(new char[]
							{
								'/'
							}) + ">")));
							break;
					...
					else
					{
						...
						{
							stringBuilder.AppendLine(text5.Substring(0, text5.IndexOf(text6)));
						}
					}
				}
			}
